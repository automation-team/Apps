...
public class GitFactory {
	...
	public void cloneWithAuthHttps(GitCredentials creds, Operation op) throws InvalidRemoteException, TransportException, GitAPIException, IOException {
		...
		final String REMOTE_URL = getFullUrl(ServiceConst.P_HTTPS, creds.getHost(), creds.getPath());
	...
	public void cloneWithAuthSsh(GitCredentials creds, Operation op) throws InvalidRemoteException, TransportException, GitAPIException, IOException {
		...
		final String REMOTE_URL = getFullUrl(ServiceConst.P_SSH, creds.getHost(),creds.getPath());
	...
	public Boolean isValid(GitCredentials gc, Integer phase) throws InvalidRemoteException, TransportException, GitAPIException{
		...
		if (phase == ServiceConst.CLONE_CHECK_CREDS_1 ){
			...
			if (gc.getProtocol().equals(ServiceConst.P_HTTPS)){
				final String REMOTE_URL = getFullUrl(ServiceConst.P_HTTPS, gc.getHost(), gc.getPath());
				...
				final String REMOTE_URL = getFullUrl(ServiceConst.P_SSH, gc.getHost(),gc.getPath());
				refs = Git.lsRemoteRepository()
	...
	private static String getFullUrl(String protocol, String host, String path){
		String suffix = "";
